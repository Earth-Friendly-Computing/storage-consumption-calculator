...
server <- function(input, output) {
    ...
    facotr_gb_to_tb = 1.0 / 1000

    
    val = reactive(
      as.numeric(input$num) * as.numeric(input$disk) * as.numeric(input$timeframe) * hours_per_year *facotr_gb_to_tb * factor_W_to_kW
      )
    output$text <- renderText({paste("Expected usage: ", val(), "kWh")})
